...
| # | Who | What |
|---|---|---|
| 1 | **Admin** | `POST /ai/parse/` with `save_to_db=true` → backend creates `Users` (role=`Employee`, password=phone) + `CVSubmission` and **sends an OTP to the seafarer's phone** via the configured SMS service |
| 2 | **Seafarer** | Receives the OTP on their phone, calls `POST /api/auth/verify-otp/` with `{phone, otp}` → marks the user as phone-verified, returns JWT |
...
No email confirmation, no Twilio required. The phone number is the credential, and the system itself sends the OTP. The default SMS backend (`ConsoleSMSService`) logs the OTP to the server console — swap in Twilio / Vonage / AWS SNS for production by setting `SMS_SERVICE` in `saker/settings.py`.
...
2. Backend creates the `Users` row with `is_phone_verified=False`, then **generates a 6-digit numeric OTP**, stores it on the row (`otp_code` + `otp_expires_at`) with a 10-minute TTL, and **sends it to the seafarer's phone** via the configured SMS service. The admin never sees the OTP in the API response.
3. **Seafarer** receives the OTP on their phone (in dev, the OTP is in the server log via `ConsoleSMSService`).
4. **Seafarer** hits `POST /api/auth/verify-otp/` with `{phone, otp}`.
...
# (OTP is sent to the seafarer's phone — not in the response)
```
...
**Why a custom backend-generated OTP (no Twilio out-of-the-box):**
...
- The default `ConsoleSMSService` logs the OTP to the server console — fine for dev/test, and any developer can read the OTP from the log.
- For production, plug in a real provider by setting `SMS_SERVICE = "your.module.YourSMSService"` in `saker/settings.py` (see `api/sms.py` for the `SMSService` protocol).
